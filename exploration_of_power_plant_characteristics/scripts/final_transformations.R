...
ResultsHistoricalData <-
	get_vom_etc(
		XCls = ClusteredHistoricalData, 
		XCleaned = CleanedHistoricalData, 
		XMisc =  MiscVariablesHistoricalData
	)
...
EternallyPresent %>%
	...
	write_csv('Results/eternally_present_output.csv')

HistoricalData %>%
	rowid_to_column %>%
	select(rowid, plant_id_eia, generator_id, report_date) %>%
	mutate(report_date = lubridate::as_date(report_date)) %>%
	inner_join(ResultsHistoricalData) %>%
	write_csv('Results/historical_output.csv')
