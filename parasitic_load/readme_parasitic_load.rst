...
Background
----------
When a power plant is activated, some of the energy that it produces is lost (`Wikipedia.org <https://en.wikipedia.org/wiki/Losses_in_electrical_systems#Parasitic_loss>`).
...
...where the denomenator seeks to normalize each plant's parasitic load based on the plant's capacity; 8,760 represents the approximate number of hours in a year.

We tested various regression model types, and settled on random forest.
Our reasons:
* Lower RMSE (root mean squared error) on our testing sets than competing models
...
* *get_y_fit()*: Fits a sci-kit learn random forest model, using the hyper parameters we already selected; then, it predicts the parasitic_load for the new data.
