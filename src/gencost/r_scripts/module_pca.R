...
# Note: Previous runs have found that CC has 3 components; GT has 2; ST has 4
# However, this script is not hard-coded, which allows for flexibility, as
# DataBySubplant may change. So, we'll find out with bootstraps how many components
# are recommended.
NestedParallelTests <-
	NestedCandidateDataBySubplant %>%
...
# (The NewData dataset will need the same columns)
VariablesToSelect <-
	NestedCandidateDataBySubplant %>%
